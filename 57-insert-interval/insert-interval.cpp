...
public:
    vector<vector<int>> insert(vector<vector<int>>& intervals,
        ...
        for (int i = 0; i < n; i++) {
            ...
            // because we have already handled the case of the intervals[i]
            ...
            // 2. CurrentStart > NewEnd
            //    -> Current interval comes after the new interval.
            else if (intervals[i][0] <= newInterval[1]) {
                // take the minimum start and the maximum end comparatively to
                // the current interval and update the new interval
                newInterval[0] = min(intervals[i][0], newInterval[0]);
                newInterval[1] = max(intervals[i][1], newInterval[1]);
            }
            // if the newInterval now doesn't overlap with anyone push it in the
            // answer array
            else {
